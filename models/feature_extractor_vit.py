...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.blocks(x)
        x = self.norm(x)
        return x[:, 1:, :] # Rest of it torch.Size([1, 144, 768])
...
def _load_weights(model: VisionTransformer, checkpoint_path: str, prefix: str = ''):
    ...
    print('[INFO] Loaded from official NPZ for Vision Transformer weights')
...
base_architecture_to_features = {'vit_tiny_patch16_224': vit_tiny_patch16_224,
...
if __name__=='__main__':
    model = base_architecture_to_features['vit_base_patch16_224'](pretrained=True, img_size=512)
