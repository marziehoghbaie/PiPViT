...
def get_patch_size():
    patchsize = 64
    skip = round((384 - patchsize) / (12-1))
    return patchsize, skip

@torch.no_grad()                    
def visualize_topk(net, projectloader, num_classes, device, foldername, k=10,save_path=None):
    print("Visualizing prototypes for topk...", flush=True)
    dir = os.path.join(save_path, foldername)
    ...
    for i, (xs, ys) in img_iter:
        ...
        with torch.no_grad():
            ...
            # out, proto_features, pooled pipvit
            # proto_features, pooled, out pipnet
            out, pfs, pooled = net(xs, inference=True)
            pooled = pooled.squeeze(0) 
            pfs = pfs.squeeze(0) 
            
            for p in range(pooled.shape[0]):
                c_weight = torch.max(classification_weights[:,p]) 
                if c_weight > weight_thresh: #ignore prototypes that are not relevant to any class
    ...
    for p in topks.keys():
        ...
        for idx, score in topks[p]:
            alli.append(idx)
            if score > pool_thresh:  #in case prototypes have fewer than k well-related patches
                found = True
        if not found:
                    ...
                    for idx, score in topks[p]:
                        if idx == i:
                            ...
                            with torch.no_grad():
                                # out, proto_features, pooled pipvit
                                # proto_features, pooled, out pipnet
                                out, softmaxes, pooled = net(xs, inference=True) #softmaxes has shape (1, num_prototypes, W, H)
                            ...
                            if (c_weight > weight_thresh) or ('pretrain' in foldername):
                                ...
                                image = transforms.Resize(size=(image_size, image_size))(Image.open(img_to_open))
                                img_tensor = transforms.ToTensor()(image).unsqueeze_(0) #shape (1, 3, h, w)
                                h_coor_min, h_coor_max, w_coor_min, w_coor_max = get_img_coordinates(image_size, softmaxes.shape, patchsize, skip, h_idx, w_idx)
    ...
    for i, (xs, ys) in img_iter: #shuffle is false so should lead to same order as in imgs
        ...
        for p in range(0, net._num_prototypes):
            ...
            if c_weight>weight_thresh:
                ...
                if found_max > pool_thresh:
                    img_to_open = imgs[images_seen_before+idx_to_select]
                    if isinstance(img_to_open, tuple) or isinstance(img_to_open, list): #dataset contains tuples of (img,label)
                        imglabel = img_to_open[1]
                        img_to_open = img_to_open[0]

                    image = transforms.Resize(size=(image_size, image_size))(Image.open(img_to_open).convert("RGB"))
                    img_tensor = transforms.ToTensor()(image).unsqueeze_(0) #shape (1, 3, h, w)
                    h_coor_min, h_coor_max, w_coor_min, w_coor_max = get_img_coordinates(image_size, softmaxes.shape, patchsize, skip, h_idx, w_idx)
                    img_tensor_patch = img_tensor[0, :, h_coor_min:h_coor_max, w_coor_min:w_coor_max]
                    saved[p]+=1
