...
def read_conf_file(config_path):
    ...
    results_path = where + '/' + log_info['save_path'] + '/' +train_config['tanh_type'] + '/' + model_name + '/' + timestr
    ...
    return model_config, dataset_info, train_config, log_info, where, config, device, check_point_path, model_name, results_path, logger


def read_dataset_conf(_path):
    file = open(_path, mode='r')
    config = yaml.safe_load(file)[0]['dataset']
    return config
